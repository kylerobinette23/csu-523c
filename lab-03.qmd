...
```{r}
county_cents <- st_centroid(us_counties) %>%
  st_union()
#plot(county_cents)
```
...
#Voroni
v_grid = st_voronoi(county_cents) %>%
  st_cast() %>%
  st_as_sf() %>%
  mutate(id = 1:n())
#plot
plot_tess(v_grid, "voronoi test")

#Triangulate
t_grid = st_triangulate(county_cents) |> 
  st_cast() |> 
  st_as_sf() |> 
  mutate(id = 1:n())
#plot
plot_tess(t_grid, "Triangle test")

#Square grid 
sq_grid = st_make_grid(us_counties, n = 70) %>%
  st_as_sf() %>%
  st_cast() %>%
  mutate(id = 1:n())
#plot
plot_tess(sq_grid,"Square test")

#Hexagonal grid
hex_grid = st_make_grid(us_counties, n = 70, square = FALSE) %>%
  st_as_sf() %>%
  st_cast() %>%
  mutate(id = 1:n())
#plot
plot_tess(hex_grid, "Hexigonal test")
```
...
```{r, eval = TRUE}
conus_boundry <- st_union(us_counties)
#plot(conus_boundry)
#Voronoi
v_grid_final <- st_intersection(v_grid, conus_boundry)
#plot
plot_tess(v_grid_final, "clipped Voronoi")
#Delaunay
t_grid_final <- st_intersection(t_grid, conus_boundry)
#plot
plot_tess(t_grid_final, "clipped Delaunay")

#Square
sq_grid_final <- st_filter(sq_grid, conus_boundry)
#plot
plot_tess(sq_grid_final, "clipped Square")
#Hexagonal
hex_grid_final <- st_filter(hex_grid, conus_boundry)
#plot
plot_tess(hex_grid_final, "clipped Hexagonal")
```
...
```{r, eval = TRUE}
conus_boundry10 = ms_simplify(conus_boundry, keep = .1)
conus_boundry5  = ms_simplify(conus_boundry, keep = .05)
conus_boundry1  = ms_simplify(conus_boundry, keep = .01) # Comfortable with this

g1 = ggplot(conus_boundry) + 
  geom_sf() + 
  labs(title = paste(mapview::npts(conus_boundry), 'points'),
       caption = "keep = 1") +
  theme_void()

g2 = ggplot(conus_boundry10) + 
  geom_sf() + 
  labs(title = paste(mapview::npts(conus_boundry10), 'points'),
       caption = "keep = .1 (10%)") +
  theme_void()

g3 = ggplot(conus_boundry5) + 
  geom_sf() + 
  labs(title = paste(mapview::npts(conus_boundry5), 'points'),
       caption = "keep = .05 (5%)") +
  theme_void()

g4 = ggplot(conus_boundry1) + 
  geom_sf() + 
  labs(title = paste(mapview::npts(conus_boundry1), 'points'),
       caption = "keep = .01 (1%)") +
  theme_void()
...
```{r}
simple_boundary <- conus_boundry1
mapview::npts(conus_boundry) #11292
mapview::npts(simple_boundary) #114 
```

Original boundary: **11292 points**

Simplified boundary: **114 points**

Removed points: **11178 points**

Computationally this will cut down on time required to compute the
intersection by reducing boundary vertices.
...
```{r}
#Voronoi
v_grid_final <- st_intersection(v_grid, simple_boundary)
#plot
#plot_tess(v_grid_final, "clipped voronoi")
#Delaunay
t_grid_final <- st_intersection(t_grid, simple_boundary)
#plot
#plot_tess(t_grid_final, "clipped Delaunay")

#Square
sq_grid_final <- st_filter(sq_grid, simple_boundary)
#plot
#plot_tess(sq_grid_final, "clipped square")
#Hexagonal
hex_grid_final <- st_filter(hex_grid, simple_boundary)
#plot
#plot_tess(hex_grid_final, "clipped Hexagonal")
...
```{r}
plot_tess(us_counties, "US Counties")
plot_tess(v_grid_final, "Voronoi Tesselation")
plot_tess(t_grid_final, "Delaunay Tesselation")
plot_tess(sq_grid_final, "Square Grid Tesselation")
plot_tess(hex_grid_final, "Hexagonal Grid Tesselation")
```
...
```{r}
summ_tess(us_counties, "Original Counties")
summ_tess(v_grid_final, "Voronoi Tesselation")
summ_tess(t_grid_final, "Delaunay Tesselation")
summ_tess(sq_grid_final, "Square Grid Tesselation")
summ_tess(hex_grid_final, "Hexagonal Grid Tesselation")
```
...
```{r}
knitr::kable(summ_table, caption = "Tesselation Characteristics",
...
The high standard deviation in area for Original Counties introduces
significant MAUP bias, as larger administrative units may appear as
hotspots simply by "trapping" more points regardless of actual density.
In contrast, Hexagonal and Square Grids eliminate this area variance
entirely SD = 0, providing a consistent spatial support that ensures
results are driven by physical dam distribution rather than the "zoning
effect" of arbitrary boundaries. While Delaunay tessellations offer
high-resolution detail, their large element count (6,198) significantly
increases computational requirements for neighbor matrix calculations
and spatial joins. Ultimately, the hexagonal grid's equal-area trait and
shared-edge geometry make it the most robust choice for minimizing MAUP
and maintaining computational efficiency in a point-in-polygon analysis.
...
plot_pip = function(data, title){
  ggplot() + 
    ...
    scale_fill_viridis_c(option = "viridis", name = "Number of Dams") + 
    theme_void() + 
    theme(#legend.position = 'none',
          plot.title = element_text(face = "bold", color = "darkgreen", hjust = .5, size = 12)) +
    labs(title = title,
         caption = paste0(sum(data$n), " Dams represented")) 
}
#Add id colmn to counties object
us_counties <- us_counties %>% 
  mutate(id = 1:n())

counties_pip <- point_in_polygon(dams2, us_counties, "id")
p1 <- plot_pip(counties_pip, "Dam Consentration: Counties")

v_pip <- point_in_polygon(dams2, v_grid_final, "id")
p2 <- plot_pip(v_pip, "Dam Consentration: Voronoi")

t_pip <- point_in_polygon(dams2, t_grid_final, "id")
p3 <- plot_pip(t_pip, "Dam Concentration: Triangulate")

sq_pip <- point_in_polygon(dams2, sq_grid_final, "id")
p4 <- plot_pip(sq_pip, "Dam Concentration: Square")

hex_pip <- point_in_polygon(dams2, hex_grid_final, "id")
p5 <- plot_pip(hex_pip, "Dam Concentration: Hexagonal")
```
...
comparison_plot <- (p1 + p2) /  
  ...
  plot_annotation(
    title = 'Dam Distribution Across Different Tessellation Methods',
    subtitle = 'Comparison of administrative boundaries vs. geometric grids',
    caption = 'Data: National Inventory of Dams | CRS: EPSG:5070',
...
**Interpretation:**\
**Geographic Divergence and Geometry**

The most striking difference occurs in the Great Plains and the Upper
Midwest. In the Counties map, the pattern appears highly fragmented and
"noisy" due to the high standard deviation in county sizes; small
counties in the East create tight clusters of color while massive
Western counties spread the data thin. In contrast, the Hexagonal
tessellation—which has a much larger mean area ($3,781$ $km^2$) and zero
area variance, smoothes this noise into a clear, continuous hotspot in
the Mississippi River Basin. Similarly, in the Mountain West, the
Triangulate (Delaunay) method creates an overly dense web of small tiles
that over-fragments the landscape, making the distribution look more
sparse than it appears in the Square grid, where the uniform support
highlights consistent regional density. These differences arise because
irregular polygons conflate the size of the "container" with the density
of the dams, a classic MAUP zoning effect.

**Recommendation for Dam Operations**

I recommend the **Hexagonal Grid** for a dam operations manager focused
on geographic clustering. Unlike counties, which are bound by political
history, hexagons provide a uniform "equal-area" support that ensures
every unit has an equal probability of capturing a dam, making it the
most mathematically honest way to identify physical density. Hexagons
are superior to square grids for hydrology because they share
equidistant borders with all six neighbors, avoiding the
"corner-touching" bias of squares and creating more reliable neighbor
matrices for the LISA analysis. While counties are the unit of
management jurisdiction, they fail to reveal the cross-border hydrologic
clusters that hexagons highlight as a singular, cohesive hotspot.

**MAUP and Management Context**

The choice of tessellation is inherently tied to the question: "Best
supports management decisions" in this context means prioritizing the
identification of physical infrastructure clusters over administrative
alignment. While counties are valid for local budget allocations, they
suffer from MAUP sensitivity that obscures the true regional scale of
dam projects. Had a finer simplification been used (a higher `keep`
percentage), the hexagons would have become smaller, potentially
over-fragmenting the clusters into "isolated" units and losing the
regional signal. For a manager, the hexagon grid strikes the perfect
balance, it is coarse enough to reveal broad regional trends in large
river systems, yet consistent enough to ensure that hotspots are a
result of infrastructure density rather than the arbitrary size of the
polygon.
...
```{r}
#Filter dams based on purpose 
water_supply_dams  <- dams2 %>% filter(
  grepl("Water Supply", purposes, ignore.case = TRUE))
recreation_dams    <- dams2 %>% filter(
  grepl("Recreation", purposes, ignore.case = TRUE))
fire_prot_dams     <- dams2 %>% filter(
  grepl("Fire Protection", purposes, ignore.case = TRUE ))
fish_wildlife_dams <- dams2 %>% filter(
  grepl("Fish", purposes, ignore.case = TRUE))

# Run Point-in-Polygon counts for each
s_counts <- point_in_polygon(water_supply_dams, hex_grid_final, "id")
r_counts <- point_in_polygon(recreation_dams, hex_grid_final, "id")
p_counts <- point_in_polygon(fire_prot_dams, hex_grid_final, "id")
f_counts <- point_in_polygon(fish_wildlife_dams, hex_grid_final, "id")
```

## Description of Chosen Purposes

I chose to analyze purposes that represent a mix of human-centric needs
(water Supply and Fire Protection) and systems designed for ecosystem
and social benefits (Fish/Wildlife and Recreation). I expect this will
produce very different spacial patters across the US.

-   **Water Supply (S):** Critical for urban and agricultural stability,
    especially in the arid West. I expect these to cluster near major
    population centers or in irrigation-heavy regions.

-   **Recreation (R):** These are often smaller, community-focused dams
    or large reservoirs used for boating and fishing. I expect a fairly
    broad distribution across the Eastern US.

-   **Fire Protection (P):** These dams are vital for emergency
    responses. Their spatial distribution might be higher in
    drought-prone areas or regions with high wildfire risks.

-   **Fish and Wildlife (F):** These dams are designed with ecological
    conservation in mind. I expect to see these concentrated in
    migratory corridors or specific conservation zones.
...
```{r}
#Water Supply Hotspots
plot_s <- plot_pip(s_counts, "Water Supply Hotspots") +
  gghighlight(n > (mean(n) + sd(n)))

#Recreation Hotspots
plot_r <- plot_pip(r_counts, "Recreation Hotspots") +
  gghighlight(n > (mean(n) + sd(n)))

#Fire Protection Hotspots
plot_p <- plot_pip(p_counts, "Fire Protection Hotspots") +
  gghighlight(n > (mean(n) + sd(n)))

#Fish/Wildlife Hotspots
plot_f <- plot_pip(f_counts, "Fish/Wildlife Hotspots") +
  gghighlight(n > (mean(n) + sd(n)))

# Arrange them using patchwork
(plot_s + plot_r) / (plot_p + plot_f) + 
  plot_layout(guides = "collect") +
  plot_annotation(
    title = "Regional Dam Hotspots by Purpose",
    subtitle = "Highlighting tiles where count > (Mean + 1 SD)"
  )
```
...
As I suspected the geographic distribution of dams is highly linked to
their purposes. For dams that include water supply as a purpose the
distribution appears to be tied to population dense areas on either
coast. There is also a cluster in the arid west which makes sense for
this water scarce area. Recreational dams appear to be concentrated to
the south and east coast region which makes sense for this purpose. The
Fire Protection and Fish/wildlife dam concentration is virtually
identical, as many of these dams are smaller and serve both purposes in
the central part of the country where fire is more common.
...
```{r}
#Join dam data with hex tessilation
tile_stats <- st_join(hex_grid_final, dams2) %>%
  st_drop_geometry() %>%
  group_by(id) %>%
  summarise(
    n_dams = sum(!is.na(nid_id)),
    mean_age = 2026 - mean(year_completed, na.rm = TRUE),
    mean_surface_area = mean(surface_area_acres, na.rm = TRUE)
  ) %>%
  #Join back to geometry
  left_join(hex_grid_final, ., by = "id") %>%
  #Replace NAs (tiles with 0 dams) with 0 for counts
  mutate(n_dams = ifelse(is.na(n_dams), 0, n_dams))

#Inspect distributions before mapping
hist(tile_stats$mean_age)
hist(log10(tile_stats$mean_surface_area))

# Map 1: Mean Dam Age
m1 <- ggplot(tile_stats) +
  geom_sf(aes(fill = mean_age), color = NA) +
  scale_fill_viridis_c(option = "viridis", name = "Age (Yrs)") +
  theme_void() +
...
m2 <- ggplot(tile_stats) +
  geom_sf(aes(fill = mean_surface_area), color = NA) +
  scale_fill_viridis_c(option = "viridis", name = "Acres", trans = "log10") +
  theme_void() +
  labs(title = "Mean Surface Area (Log10)")

m2

# Map 3: Dam Count 
m3 <- ggplot(tile_stats) +
  geom_sf(aes(fill = n_dams), color = NA) +
  scale_fill_viridis_c(option = "viridis", name = "Count") +
  theme_void() +
  labs(title = "Dam Count")
...
For your tessellation's neighbor matrix compute the **spatial lag** of
dam attributes: the average value of an attribute in a tile's neighbors.
This reveals whether *spatial neighbors have similar dam attributes*—a
key signal that observations are **not independent** (violating OLS
assumptions).
...
touch_matrix <- st_touches(tile_stats, sparse = FALSE)
#Covert to 1s and 0s
W <- touch_matrix * 1
#Row-Standardize
row_sums <- rowSums(W)
W_std <- W / ifelse(row_sums == 0, 1, row_sums)

#Prepare vectors for the attributes
counts <- tile_stats$n_dams
age    <- tile_stats$mean_age
size   <- tile_stats$mean_surface_area

#Handle NAs via Global Mean Imputation
age_imputed <- ifelse(is.na(age) | is.nan(age), mean(age, na.rm = TRUE), age)
size_imputed <- ifelse(is.na(size) | is.nan(size), mean(size, na.rm = TRUE), size)

#Compute Spatial Lag (W_std %*% attribute)
lag_counts <- as.numeric(W_std %*% counts)
lag_age    <- as.numeric(W_std %*% age_imputed)
lag_size   <- as.numeric(W_std %*% size_imputed)

# Correlation (using only tiles with original data)
# We filter to tiles where n_dams > 0 so we aren't correlating imputed noise
valid_idx <- which(counts > 0)

cor_counts <- cor(counts[valid_idx], lag_counts[valid_idx], use = "complete.obs")
cor_age    <- cor(age[valid_idx], lag_age[valid_idx], use = "complete.obs")
cor_size   <- cor(size[valid_idx], lag_size[valid_idx], use = "complete.obs")

#Summary Table
# Create the data frame for the table
summary_df <- data.frame(
  Attribute = c("Dam Count", "Mean Age", "Mean Size (Area)"),
  Correlation = c(cor_counts, cor_age, cor_size)
) %>%
  mutate(
    Strength = case_when(
...
summary_df %>%
  mutate(Correlation = round(Correlation, 3)) %>% # Round for clean display
  kable(
    col.names = c("Dam Attribute", "Spatial Lag Correlation (r)", "Strength Label"),
    caption = "Spatial Autocorrelation of Dam Attributes",
    align = "lcc", # Left-align first col, center the rest
    booktabs = TRUE # Professional spacing
  ) %>%
  kable_styling(
  ...
  # Add some color to the 'Strong' rows for visual emphasis
  column_spec(3, bold = TRUE, 
              color = ifelse(summary_df$Strength == "Strong", "white", "black"),
              background = ifelse(summary_df$Strength == "Strong", "#2E8B57", "transparent")) %>%
  row_spec(0, bold = TRUE) # Bold the header row
```

**Interpretation:**

The strong correlation observed in dam counts and mean age suggests that
dam density is heavily influenced by regional geography and hydrology.
Additionally nearby dams were build around the same time. This
clustering indicates a violation of the independence assumption,
necessitating the use of spatial regression techniques to account for
neighbor effects.
...
```{r}
#Standardize counts
z_attr <- as.vector(scale(counts))
z_lag  <- as.vector(scale(lag_counts))
#Assign HH / LL / HL / LH quadrants
# HH: High value, High neighbor average (Hotspot)
# LL: Low value, Low neighbor average (Coldspot)
moran_df <- data.frame(z_attr = z_attr, z_lag = z_lag) %>%
...
#Moran plot
moran_plot <- ggplot(moran_df, aes(x = z_attr, y = z_lag, color = quadrant)) +
  geom_vline(xintercept = 0, linetype = "dashed", alpha = 0.5) +
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", color = "black", se = FALSE) + # The slope is Moran's I
  scale_color_manual(values = c("HH" = "red", "LL" = "blue", "HL" = "orange", "LH" = "purple")) +
  labs(
    title = "Moran Scatterplot: Dam Count",
    subtitle = paste0("Spatial Correlation (r) = ", round(cor_counts, 3)),
    x = "Standardized Dam Count (z)",
    y = "Standardized Spatial Lag (Wz)",
    color = "Quadrant"
  ) +
  theme_light()

moran_plot
#Report quadrant counts
table(moran_df$quadrant)
```

**Interpretation:**

The scatterplot reveals a strong positive slope, with the majority of
points concentrating in the HH and LL quadrants. This visual alignment
along the diagonal line confirms the correlation calculated in Step 5.2.
Because points are not evenly dispersed across all four quadrants, we
can conclude that dam attributes exhibit strong spatial dependence,
where a tile's value is a reliable predictor of its neighbor's value.
This pattern suggests that dam infrastructure is driven by
regional-scale geographic or political factors rather than independent
local decisions.
...
The statistical analysis reveals that dam infrastructure in the United
States is far from a random distribution; instead, it exhibits a
distinct, highly organized spatial structure. According to the results
from Step 5.2, Dam Count shows the most significant signal of spatial
dependence with a correlation of $r = 0.829$. This value indicates a
very strong positive spatial autocorrelation, confirming that
high-density clusters are not isolated incidents but regional phenomena.
In contrast, while Mean Age and Mean Size (surface area) typically show
low to moderate correlations ($r \approx 0–0.6$), they rarely reach the
intensity of raw counts. This suggests that while "when" and "how large"
a dam was built is influenced by regional trends, the sheer density of
infrastructure is the most geographically "clumpy" characteristic of our
hydraulic landscape.

The Moran Scatterplot for dam counts visually validates this strong
quantitative finding. The vast majority of observations are tightly
packed along the positive diagonal, primarily within the LL (Coldspot)
and HH (Hotspot) quadrants. With 1,486 tiles in the LL quadrant and 523
in the HH quadrant, the landscape is clearly segmented with large swaths
of the country (likely the Great Basin and High Plains) having almost no
dam infrastructure, while specific river basins host intense,
interconnected networks. The steep slope of the regression line
($r = 0.829$) acts as a global summary of this pattern, if you know the
dam density of a specific tile, you can predict the density of its
neighbor with high accuracy. The relatively few points in the HL and LH
quadrants (113 and 169 respectively) suggest that "spatial outliers",
such as a lone, high-density tile in a desert, are rare exceptions
rather than the rule.

These patterns are deeply rooted in the intersection of hydrology and
historical policy. The strong clustering of dam counts and age reflects
the "project-based" nature of American infrastructure. For example, the
Tennessee Valley Authority (TVA) in the 1930s and the Bureau of
Reclamation projects in the West during the 1960s were not individual
efforts but massive, regional-scale deployments. Neighboring tiles share
the same topographic constraints (e.g., steep canyons or major river
stems) and the same political mandates, leading to the highly
synchronized spatial structure seen in the HH quadrant.

This global analysis directly complements the findings from Question 4,
where we observed that dam purposes (like irrigation in the West vs.
flood control in the East) follow rigid geographic boundaries. The Moran
analysis confirms that these purposeful clusters are statistically
significant and regional in scale. For a manager, this implies that dam
operations and ecosystem impacts cannot be studied in isolation.
...
```{r}
#Clip geometry
hex_cropped <- st_filter(hex_grid_final, conus_boundry)
county_units <- us_counties
#Recompute dam counts
hex_dams <- point_in_polygon(dams2, hex_cropped, "id")
county_dams <- point_in_polygon(dams2, county_units, "id")
```
...
```{r}
#Create neighbor adjacency matrix
W <- st_touches(hex_dams, sparse = FALSE) * 1
diag(W) <- 0
row_sums <- rowSums(W)
W_std <- W / ifelse(row_sums == 0, 1, row_sums)
#Compute lag
dam_counts <- as.numeric(hex_dams$n)
lag_counts <- as.numeric(W_std %*% dam_counts)
#Standardize z
hex_dams <- hex_dams %>%
  mutate(
    z_n = as.vector(scale(n)),
    z_lag = as.vector(scale(lag_counts))
  )
...
#Create function for Lisa analysis
run_lisa_analysis <- function(data_sf, unit_name) {
  #Create neighbor adjacency matrix
  W <- st_touches(data_sf, sparse = FALSE) * 1
  diag(W) <- 0
  row_sums <- rowSums(W)
  W_std <- W / ifelse(row_sums == 0, 1, row_sums)
  #Compute lag
  dam_counts <- as.numeric(data_sf$n)
  lag_counts <- as.numeric(W_std %*% dam_counts)
  #Standardize z
  data_sf <- data_sf %>%
    mutate(
...
lisa_sf <- data_sf %>%
  mutate(quad_label = case_when(
    z_n > 0  & z_lag > 0  ~ "HH (Hotspot)",
    z_n < 0  & z_lag < 0  ~ "LL (Coldspot)",
    z_n > 0  & z_lag < 0  ~ "HL (High-Low Outlier)",
    z_n < 0  & z_lag > 0  ~ "LH (Low-High Outlier)",
    TRUE ~ "Insignificant" # Catch-all for zeros if necessary
  ))

#Create the LISA Map
p <- ggplot(lisa_sf) +
  geom_sf(aes(fill = quad_label), color = "white", size = 0.05) +
  scale_fill_manual(
    values = c(
      "HH (Hotspot)" = "red", 
      "LL (Coldspot)" = "blue", 
      "HL (High-Low Outlier)" = "yellow", 
      "LH (Low-High Outlier)" = "cyan"
    ),
   drop = FALSE # Keep all categories in legend
    ) +
    theme_void() +
    labs(title = paste("LISA:", unit_name)) +
    theme(legend.position = "none") # Remove individual legends for patchwork
  
  #Return a list with the data and the plot
  return(list(data = lisa_sf, plot = p))
}

hex_lisa_results <- run_lisa_analysis(hex_dams, "Hexagon Tessellation")
hex_lisa_results$plot
```
...
```{r}
#Run lisa function for Counties
county_results <- run_lisa_analysis(county_dams, "County Units")

#Side by side map
hex_lisa_results$plot + county_results$plot +
  plot_layout(guides = "collect") &
  plot_annotation(
    title = "Comparative LISA Analysis: Hexagons vs. Counties",
    subtitle = "Identifying Spatial Hotspots and MAUP Sensitivity in US Dam Density",
    caption = "Data Source: National Inventory of Dams (NID)",
    theme = theme(
...
hex_counts <- table(hex_lisa_results$data$quad_label)
county_counts <- table(county_results$data$quad_label)

print("Hexagon Quadrants:")
print(hex_counts)
print("County Quadrants:")
print(county_counts)
...
Comparing the Hexagonal tessellation with the County units we see that
HL and LH outliers are more rare in the hexagonal grid. This is because
tiles are uniform compared to the County plot where western counties are
much larger than eastern ones. This administrative boundary helps to
creates outliers that aren't present in the equal area hexagonal grid.
However, there are robust sections where the HH Hotspots are strongly
correlated in both maps. Specifically the central and southeast region,
meaning the clustering is so strong that it doesn't matter which
container you use to measure, the geography is speaking louder than the
administrative lines. Ultimately, the hexagonal grid is best for seeing
true physical distribution of dams since there is no bias from
population density and political borders while the counties plot is best
...
The side-by-side LISA analysis provides a definitive look at the
"hydraulic backbone" of the United States. In both the hexagon and
county models, the most prominent High-High (HH) Hotspot is anchored in
the Southeast (TVA region) and extends up the mid-Atlantic coast. Other
significant hotspots appear in the Northern Plains and along the
Missouri-Mississippi river corridor. These clusters represent areas
where dam infrastructure is functionally dense and regionally
integrated. Conversely, the Low-Low (LL) Coldspots dominate the Great
Basin, the Midwest, and the desert Southwest. These patterns are driven
by a combination of high-discharge river systems (Southeast) and intense
agricultural irrigation needs (West), contrasted against the arid,
topographically challenging, or federally protected lands of the
interior West where development is sparse.

Taking the Tennessee Valley Authority (TVA) region as a specific HH
hotspot, three primary factors explain this intense clustering. First,
topography and hydrology: the region's high annual rainfall and steep
elevation changes in the Appalachian foothills provided ideal conditions
for hydroelectric power generation. Second, federal policy: the TVA Act
of 1933 mandated a centralized, basin-wide approach to flood control and
navigation, leading to a highly synchronized network of dams. Third,
economic development: the density was further increased by the
mid-20th-century push for rural electrification and industrial cooling
water, creating a feedback loop where infrastructure drew in industry,
requiring even more infrastructure.

The comparison between the Hexagon and County units serves as a
masterclass in the Modifiable Areal Unit Problem (MAUP) at a local
scale. In the hexagon grid, the hotspots appear as cohesive, organic
"blobs" that follow river basins. However, in the County map, the
hotspots are often jagged or fragmented by administrative lines. A clear
"Zoning Effect" is visible in the West: large, sparsely populated
counties are frequently flagged as HL or LH anomalies (yellow and cyan).
For instance, a single massive county containing a few major dams might
be flagged as an outlier simply because it borders vast, empty counties,
whereas the equal-area hexagon grid reveals these areas to be part of a
much more uniform, sparse distribution.

For dam operations and policy, these LISA outputs are highly actionable.
HH Hotspots represent the most critical areas for multi-state
coordination; because these dams are so spatially dependent, an
operational change or failure at one site has a "neighborhood effect" on
the others. This aligns with the findings in Question 4, where
irrigation and hydroelectric clusters were shown to cross state lines.
LL Coldspots allow for simpler, more localized management structures due
to the lack of neighbor interference. Finally, the HL/LH outliers
identify "policy islands", places where high-density infrastructure
exists in isolation. These sites require special regulatory attention,
as they lack the regional support networks found in the major hotspots
but may face unique local pressures.
