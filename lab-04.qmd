...
```{r}
#Define AOI with bounding box
bbox_palo <- AOI::geocode("Palo, Iowa", bbox = TRUE)
#Define flood window
time_range <- "2016-09-24/2016-09-29"
#Search catalog
items <- stac_query %>%
  stac_search(
...
#Select bands 1-6
bands_to_get <- c('coastal', 'blue', 'green', 'red', 'nir08', 'swir16')

#Start Download 
downloaded_files <- assets_download(
  items = item_signed,
  asset_names = bands_to_get,
  output_dir = "data/palo_flood",
  overwrite = TRUE
)

#Build Raster object
files <- list.files( path = "data/palo_flood",
  pattern = "LC08.*\\.TIF$",
  recursive = TRUE, 
  full.names = TRUE)

#files
raster_files <- sort(files)[1:6]
#raster_files
#Stacked SpatRaster
flood_raster <- rast(raster_files)
#SetNames
band_names <- c('coastal', 'blue', 'green', 'red', 'nir08', 'swir16')
flood_raster <- setNames(flood_raster, band_names)
```
...
#Crop
palo_flood_crop <- crop(flood_raster,palo_utm)
#plotRGB(palo_flood_crop, r = 4, g = 3, b = 2, stretch = "lin", 
...
plotRGB(palo_flood_crop, r = 4, g = 3, b = 2, stretch = "lin", 
        main = "Natural Color: Red-Green-Blue")
#NIR_RG plot
plotRGB(palo_flood_crop, r = 5, g = 4, b = 3, stretch = "lin", 
        main = "color infared: NIR-R-G ")
#false color water focus plot
plotRGB(palo_flood_crop, r = 5, g = 6, b = 4, stretch = "lin", 
        main = "false color water focus: NIR-SWIR1-R")
#Agriculture/vegetation focus
plotRGB(palo_flood_crop, r = 6, g = 5, b = 2, stretch = "lin", 
        main = "false color Vegetation focus: SWIR1-NIR-B")
```

**R-G-B Natural Color:** This mimics what you would naturally see and is good for general orientation and identifying man-made structures.

**NIR-R-G Color Infrared**: Vegetation appears in shades of bright red, with healthier and denser plants reflecting deeper red. In this context the red areas show surviving crops/forests, while the dark edges show exactly whee the water has encroached on the fields.

**NIR-SWIR-R Water Focus**: This combination highlights the transition between land and moisture. Water absorbs both NIR and SWIR light, making it stand out as a deep, dark void. This is best for mapping moist soils, helping to identify flooded areas.

**SWIR-NIR-B Vegetation Focus:** This combination is particularly effective at highlighting the contract between healthy green plants and dark flooded zones.
...
#NDVI
ndvi <- (palo_flood_crop$nir08 - palo_flood_crop$red) / (palo_flood_crop$nir08 + palo_flood_crop$red)
#NDWI
ndwi <- (palo_flood_crop$green - palo_flood_crop$nir08) / (palo_flood_crop$green + palo_flood_crop$nir08)
#MNDWI
mndwi <- (palo_flood_crop$green - palo_flood_crop$swir16) / (palo_flood_crop$green + palo_flood_crop$swir16)
#WRI
wri <- (palo_flood_crop$green + palo_flood_crop$red) / (palo_flood_crop$nir08 + palo_flood_crop$swir16)
#SWI
swi <- 1 / (sqrt(palo_flood_crop$blue - palo_flood_crop$swir16))

#Combine and set names
indices <- c(ndvi, ndwi, mndwi, wri, swi)
names(indices) <- c("NDVI", "NDWI", "MNDWI", "WRI", "SWI")
...
>
> **NDVI**: Land/crops appear red (high values) while water reflects blue (negative values), This is the inverse of the water indices.
>
> **NDWI**: Water appears red while land is blue, good for open water not as good at identifying wet soils.
>
> **MNDWI**: Water is bright red while land/buildings are deep blue. By replacing NIR with SWIR this index is much better at suppressing noise from the town.
>
> **WRI**: Water values generally \> 1, More dramatic distinction between water and surrounding area.
>
> **SWI**: This isolated just the water and most saturated areas.
...
```{r}
#Apply thresholds to create binary maps (1 = Flood, 0 = Dry)
ndvi_binary  <- app(ndvi,  function(x) ifelse(x < 0, 1, 0))
ndwi_binary  <- app(ndwi,  function(x) ifelse(x > 0, 1, 0))
mndwi_binary <- app(mndwi, function(x) ifelse(x > 0, 1, 0))
wri_binary   <- app(wri,   function(x) ifelse(x > 1, 1, 0))
swi_binary   <- app(swi,   function(x) ifelse(x < 5, 1, 0))

#Stack and Name
binary_stack <- c(ndvi_binary, ndwi_binary, mndwi_binary, wri_binary, swi_binary)
names(binary_stack) <- c("NDVI_Flood", "NDWI_Flood", "MNDWI_Flood", "WRI_Flood", "SWI_Flood")
#set NA to 0
binary_stack_clean <- app(binary_stack, function(x) {
  x[is.na(x)] <- 0
  return(x)
})
#plot
plot(binary_stack_clean, 
     col = c("white", "blue"), 
     main = names(binary_stack_clean))
...
The five flood maps demonstrate a high degree of spatial consistency, all successfully identifying the primary channel of the Cedar River and the significant inundation of the agricultural fields and low-lying areas surrounding Palo. The **NDVI**, **NDWI**, **WRI**, and **SWI** maps appear nearly identical in their flood extent, suggesting that for this specific high-magnitude event, the contrast between water and healthy vegetation is sharp enough that simple band ratios converge on a similar result. However, the **MNDWI** map deviates significantly by showing a much "noisier" and more expansive flood footprint, particularly in the center-left portion of the scene. This is likely due to MNDWI’s sensitivity to the **SWIR** band; while this usually helps suppress urban noise, in this context it appears to be flagging saturated soils or specific crop types as standing water, leading to a broader and more fragmented classification of the flood zone compared to the more conservative estimates of the other four indices.
...
extracted_values <- values(palo_flood_crop)
#Check dimensions
dim(extracted_values)
```

These dimensions tell you that the data has been transformed from a geospatial object into a spectral table. Each row in this table represents a single point on the ground in Palo, and each column represents how bright that specific point is in a different wavelength of light.

-   Remove NA values from your extracted data with `na.omit` for safety

```{r}
clean_values <- na.omit(extracted_values)
dim(clean_values)
```
...
-   Once the `kmeans` algorithm runs, the output will be a list of components. One of these is `cluster` which provides a vector of integers from (1:`k`) indicating the cluster to which each row was allocated.
    ...
    k_clusters <- 12
    flood_kmeans <- kmeans(clean_values, centers = k_clusters, iter.max = 100)
    #extract cluster
    cluster_vector <- flood_kmeans$cluster
    head(cluster_vector)
    ```
...
```{r}
# Copy band 1
kmeans_raster <- palo_flood_crop[[1]]
values(kmeans_raster) <- NA
#Identify which cells in the original raster are NOT NA
valid_cells <- !is.na(values(palo_flood_crop[[1]]))
#Set the values of blank template
values(kmeans_raster)[valid_cells] <- cluster_vector

# Plot the result
# Using 12 distinct colors to see the different spectral groupings
plot(kmeans_raster, 
       col = rainbow(k_clusters), 
       main = paste("K-Means Classification (k =", k_clusters, ")"))
...
for (k in k_values) {
  # 1. Run K-means
  set.seed(2026)
  fit <- kmeans(clean_values, centers = k, iter.max = 100)
  
  # 2. Create the result raster
  res_raster <- palo_flood_crop[[1]]
  values(res_raster) <- NA
  
  # 3. Fill values
  valid_cells <- !is.na(values(palo_flood_crop[[1]]))
  values(res_raster)[valid_cells] <- fit$cluster
  
  # 4. Plot
  plot(res_raster, 
       col = rainbow(k), 
       main = paste("K-Means Classification (k =", k, ")"))
}
```
...
contingency_table <- table(values(mndwi_binary), values(kmeans_raster))
#Find flooded
flood_cluster_id <- which.max(contingency_table[2, ])
print(paste("The cluster that best represents flood water is:", flood_cluster_id))
#Extract that specific cluster as a new binary mask
#If the pixel equals flood_cluster_id, set it to 1, otherwise 0
kmeans_flood_mask <- app(kmeans_raster, function(x) ifelse(x == flood_cluster_id, 1, 0))

#Clean up NAs to 0 for a clean stack
kmeans_flood_mask <- app(kmeans_flood_mask, function(x) {
  x[is.na(x)] <- 0
  return(x)
})

#Add to existing stack and plot
indices_stack_final <- c(binary_stack_clean, kmeans_flood_mask)
names(indices_stack_final)[6] <- "KMeans_Detected"

# Plot all 6 to compare!
plot(indices_stack_final, col = c("white", "blue"))
```
...
```{r}
#Sum up all the 1s (flooded pixels) for each layer
pixel_counts <- global(indices_stack_final, fun = "sum", na.rm = TRUE)

#Convert counts to area
# Area (m^2) = count * (30 * 30)
# Area (km^2) = Area (m^2) / 1,000,000
resolution <- 30
flood_area_km2 <- (pixel_counts * (resolution^2)) / 1000000

#Print the results
colnames(flood_area_km2) <- "Area_km2"
print(round(flood_area_km2, 2))
```
...
```{r}
#Sum the entire 6-layer binary stack
flood_consensus <- app(indices_stack_final, sum)

#Plot using the blues9 palette
plot(flood_consensus, 
     col = blues9, 
     main = "Flood Classification Consensus (0-6 Models)")
```
...
```{r}
#Copy the consensus raster
consensus_interactive <- flood_consensus

#Set 0 to NA (so we only see the flooded areas on the map)
consensus_interactive[consensus_interactive == 0] <- NA

#Launch mapview
library(mapview)
mapview(consensus_interactive, 
        col.regions = blues9, 
        layer.name = "Model Consensus (1-6)")
```
